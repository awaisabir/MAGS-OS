...
string rtc_fetch_time()
{
	...
	cached_time = t;
   	...
   	string time = strcat(int_to_string(t.hour), ":");
	time = strcat(time, int_to_string(t.minute));
	time = strcat(time, ":");
	time = strcat(time, int_to_string(t.second));
	
	result = time;
	return result;
}

string rtc_fetch_date()
{
	struct rtc_time t;
	string result;
	...
	do {
		...
		t.month = rtc_read_port(RTC_MONTH);
		t.year = rtc_read_port(RTC_YEAR);
	} while(t.day != rtc_read_port(RTC_DAY_OF_MONTH));
	...
	cached_time = t;
   	...
   	string date = strcat(int_to_string(t.day), "/");
	date = strcat(date, int_to_string(t.month));
	date = strcat(date, "/");
	date = strcat(date, int_to_string(t.year));
